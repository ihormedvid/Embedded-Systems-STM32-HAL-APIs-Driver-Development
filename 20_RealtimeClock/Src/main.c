...
void rtc_init(void) {
	...
	RCC_OscInitTypeDef			RCC_OscIntStruct;
	RCC_PeriphCLKInitTypeDef	PeriphClkIniStruct;

	// Turn of LSE and turn on LSI

	RCC_OscIntStruct.OscillatorType = RCC_OSCILLATORTYPE_LSI | RCC_OSCILLATORTYPE_LSE;
	RCC_OscIntStruct.LSEState = RCC_LSE_OFF;
	RCC_OscIntStruct.LSIState = RCC_LSI_ON;

	HAL_RCC_OscConfig(&RCC_OscIntStruct);

	// Set RTC clock src to LSI

	PeriphClkIniStruct.PeriphClockSelection = RCC_PERIPHCLK_RTC;
	PeriphClkIniStruct.RTCClockSelection =  RCC_RTCCLKSOURCE_LSI;

	HAL_RCCEx_PeriphCLKConfig(&PeriphClkIniStruct);
